...
- **Current objective**: HTTP API 仕様（OpenAPI）とイベント仕様（AsyncAPI）を確定し、全契約項目を REQ にマッピングする
- **Definition of Done**:
...
| completeAllQueue | POST /queue/complete-all | REQ-0038 | - |
...
| searchTaskIds | GET /tasks/search-ids | REQ-0039 | - |
| searchTasks | GET /tasks/search | REQ-0005, REQ-0011, REQ-0012, REQ-0048 | - |
...
- Deliverable type: openapi, asyncapi
- Target file path: `30_contract/openapi.yaml`, `30_contract/asyncapi.yaml`
- Required sections to modify:
  - (See work steps below)

#### 8.4.2 Work steps (Follow in order)
1. Focus REQs（Section 6）から必要な API エンドポイントを抽出
2. openapi.yaml を作成：operationId + x-requirements を必ず設定
3. asyncapi.yaml を作成：messageId + x-requirements を必ず設定
4. gen_bundle.sh を実行して Contract Snapshot を更新
5. Section 9 の Pre-flight Checks で漏れがないか確認
...
- Must keep operationId stable: Yes (stability required)
- Must keep messageId stable: Yes (version management: *.v1, *.v2)
- Must not introduce new terms unless added to glossary first: Yes
- Must update traceability: No (Design phase)
---

## 9) Pre-flight Checks (Gate)
<!-- AUTO:BEGIN preflight_checks -->
### 9.1 Coverage Checks
- [x] All operationIds have x-requirements (21/21)
